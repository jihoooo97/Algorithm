...
class GameMap {
	...
	public void setBuildingsAt(int r, int c, int value) {
		...
		this.buildings[r][c] = value; // 칸에 데이터를 삽입
		this.rowSums[r] += value;
		this.colSums[c] += value;
		this.diagonalSumA[r + c] += value;
		this.diagonalSumB[c - r + rows - 1] += value;
	}
	...
	public int getTotalPointAt(int r, int c) {
		int result = this.rowSums[r] + this.colSums[c] + this.diagonalSumA[r + c] + this.diagonalSumB[c - r + rows - 1];
		result -= getBuildingsAt(r, c) * 3;
		return result;
	}
